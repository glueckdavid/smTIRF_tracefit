...
#IDEA:
#maybe use "df.reset_index()" to "restore" inidces?
...
def create_dataframe_new(file_path):
    ...
    #Find where the data starts by searching "ID" -------------------------------------

    start_data_list = []

    for column in dataframe.columns:
        start_data = dataframe[dataframe[column] == "ID"].index
    ...
    start_data = max(start_data_list)

    #Creating a dataframe for data and header ----------------------------------------

    if start_data == 0:
        header_dataframe = pd.DataFrame()
        data_dataframe = dataframe
        #column_names = dataframe.loc[dataframe.index == 0].values.flatten().tolist()

    else: 
        header_dataframe = dataframe[0:start_data][dataframe.columns[0:2]]
        data_dataframe = dataframe[start_data+1:]
        column_names = dataframe.loc[dataframe.index == start_data].values.flatten().tolist()
    ...
    # print("data_dataframe[0:10]")
    # print(data_dataframe[0:10])

    # print("header_dataframe[0:10]")
    # print(data_dataframe[0:10])

    ID_set = set()

    amount_rows = len(data_dataframe)

    for row in range(amount_rows):
        ID_set.add(int(data_dataframe.iloc[row]["ID"]))

    amount_IDs = len(ID_set)
    print(f'   -> There is/are {amount_IDs} different ID/s:')
    print(f"      {ID_set}")
    print(f"   -> There are {amount_rows} rows\n")

    print("______________________________________________________")
    print("Header Section:")
    print("______________________________________________________")

    if header_dataframe.empty:
        print('   -> The Header Section in the CSV file is empty')
    else:
...
def adjust_dataframe_global(data_section, header_section, local_parameters_template, local_parameters_path):
    ...
    try:
        ...
        frames_per_second = input('"Frames per second" could not be found in the header. Please enter value in [1/s]')
...
def adjust_dataframe_local(data_section, local_parameters_path):
    ...
    print("______________________________________________________")
    print("The following local adjustments were made:")
    print("______________________________________________________")
    ...
    for current_ID in ID_set:
        ...
        for row in range(len(iter_data_section)):
            ...
            if bleaching_counter >= bleaching_range:
                ...
                output_data_section["Frequency Donor"][first_row_org_df[0] : last_row_org_df[0]+1] = 0
                output_data_section["Frequency Acceptor"][first_row_org_df[0] : last_row_org_df[0]+1] = 0
                output_data_section["FRET efficiency"][first_row_org_df[0] : last_row_org_df[0]+1] = 0
                ...
                #Why do we need +1 at last rows? (and maybe -1 in the beginning?)
    ...
    for current_ID in ID_set:
        ...
        for row in range(len(iter_data_section)):
            ...
            if all([ values == 0.0 for values in list_of_values]) and list_of_values != [] and len(list_of_values) == blinking_value:
                ...
                output_data_section["FRET efficiency"][first_row_org_df[0] : last_row_org_df[0]] = 0
                #hier auch noch +1 wie oben?
    ...
    for row in range(len(output_data_section)):
        ...
        #modulu = 0 funktioniert nicht
            ...
            if math.isnan(data_means["ID"]):
                print(f'NAN detected in ID {ID} at line{row}')
                continue
            #letzter bin ggf weniger als x zeilen - average wird nan
            #ist das noch notwendig?

            
            data_bin = data_bin.astype(int) 
            #für den Vergleich der Bin IDs allerdings wieder ints
            #Data bin selbst wird außer für Vgl nicht mehr gebraucht!           

            #if float(data_means["ID"]) != data_bin["ID"].iloc[0]:
            ...
            # if not int(data_means["ID"]) == all(data_bin["ID"]):          
            #     counter = counter +1
            #     print(f"   -> 'Continue' during blinking at ID {current_ID} and {output_data_section.iloc[row]['Time']} s")
            #     continue

            crit_value = 0

            for element in data_bin["ID"]:
                if int(data_means["ID"]) != element:
...
def histogram_fit (noises, binsize, figsize, cutoff = 68.2):
...
def find_transition_points(liste_y, sigma, transition_search_loops, transition_treshold):
    ...
    #plot in gewünschter größe für später erstellen
...
def find_levels(liste_y, transition_points, blending_cycles, max_diff):
    ...
    transition_points_indices = []

    for i,transition_point in enumerate(transition_points):
        transition_points_indices.append(transition_points[i]["index"])
    print(f'Transitionpoints indices: {transition_points_indices}')

    transition_points_indices = np.unique(transition_points_indices)
    # eigentlich für sets
    print(f'Transitionpoints indices without doubles: {transition_points_indices}')

    transition_points_ordered = sorted(transition_points_indices)
    print(f'Transitionpoints ordered: {transition_points_ordered}') 

    for i, element in enumerate(transition_points_ordered):
        start = element
        print("START:")
        print(start)
        ...
        try:
            end = transition_points_ordered[i+1]
        except IndexError:
            end = -1
    ...
    plt_1 = plt.figure(figsize=(20, 7))    
    lol = histogram_fit (means_for_histo, binsize, 20, cutoff = 68.2)
    plt.show()
